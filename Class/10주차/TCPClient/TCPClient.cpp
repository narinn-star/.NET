...
int main(int argc, char* argv[])
{
	...
	// socket()
	SOCKET sock = socket(AF_INET, SOCK_STREAM, 0);	//*soket 이용해서 전화기 하나 샀다고 생각하면됨	// socket(각 옵션들)
	...
	//* 위 한줄이 전화번호 넣는거
	retval = connect(sock, (SOCKADDR *)&serveraddr, sizeof(serveraddr));//* 전화기로(sock) 얻은 전화번호(serveraddr)로 전화 건다는 의미
