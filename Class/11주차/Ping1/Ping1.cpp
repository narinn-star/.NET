...
typedef struct _IPHEADER
{
	...
	IN_ADDR ip_dst;   // destination address	//203.241.228.129 넣을 거임
} IPHEADER;
...
int main(int argc, char* argv[])
{
	...
	for(int i=0; i<4; i++){
		...
		icmpmsg.icmp_seq = i;
		icmpmsg.icmp_cksum = checksum(
			(u_short *)&icmpmsg, sizeof(icmpmsg));

		// 에코 요청 ICMP 메시지 보내기
		retval = sendto(sock, (char *)&icmpmsg, sizeof(icmpmsg),	//send 하고
		...
		retval = recvfrom(sock, buf, BUFSIZE, 0,					//receive 하기
