...
int main(int argc, char* argv[])
{
	...
	serveraddr.sin_port = htons(9000);//* port 번호
	...
	while(1){//* 사용자가 접속하면 while문 안으로 들어감
		...
		while(1){//* 여기 서버는 누구 한명이 접속을 하면 while문에서 계속 갇혀있음. 근데 들어와있는 사람이 다시 나가면 위에 있는 while문 내부에 있는 client_sock으로 다시 가서 block되어있고 새로운 
			...
			// 데이터 보내기
			retval = send(client_sock, buf, retval, 0);//* server는 받은 데이터를 그대로 다시 client 에게 보여주고 다시 위로 recv로 가서 또 block된상태로 기다린다(82번째줄)
			if(retval == SOCKET_ERROR){
				err_display("send()");
				break;
			}
